...
import ch.uzh.ifi.ce.cabne.execution.modifiedclasses.HackyPWLBRCalculator;
import ch.uzh.ifi.ce.cabne.execution.modifiedclasses.HackyRandomGenerator;
...
public abstract class LLGExperimentBase {
	...
	public static int seedFromIndex(int runIndex) {
		return new Random(runIndex).nextInt((int) 1e9);
	}
	
	public static void configureAlgorithm(String algorithmName, BNESolverContext<Double, Double> context, int rngSeed) {
		...
		case "2quasi":
			...
			context.setRng(2, new HackyRandomGenerator(2, rngSeed));
			context.setRng(1, new HackyRandomGenerator(1, rngSeed));
			context.setOptimizer(brentSearch);
			context.setUpdateRule(linearUpdate);
			context.setBRC(new HackyPWLBRCalculator(context));
			break;
		case "2bquasinohack":
			context.activateConfig("quasi");
			context.setRng(2, new QuasiRandomGenerator(2, rngSeed));
			context.setRng(1, new QuasiRandomGenerator(1, rngSeed));
			context.setOptimizer(brentSearch);
			context.setUpdateRule(linearUpdate);
			context.setBRC(new PWLBRCalculator(context));
			break;
		case "3common":
			...
			context.setRng(2, new CommonRandomGenerator(2, rngSeed));
			context.setRng(1, new CommonRandomGenerator(1, rngSeed));
			context.setOptimizer(brentSearch);
			context.setUpdateRule(linearUpdate);
			context.setBRC(new PWLBRCalculator(context));
			break;
