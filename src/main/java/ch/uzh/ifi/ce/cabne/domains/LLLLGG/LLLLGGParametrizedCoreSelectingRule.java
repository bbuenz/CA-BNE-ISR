    ...
    private void createWeightedReferenceObjective(MIP mip, Variable[] variables, double[] vcg, double[] IR, double minrev) {
        ...
        switch (weightName) {
            case "vcgpayoff":
                ...
                for (int i = 0; i < 6; i++) {
                    inverseWeights[i] = 1.0 / Math.max(IR[i] - vcg[i], EPSILON);
                }
                break;
            case "vcgpayments":
                inverseWeights = new double[6];
                for (int i = 0; i < 6; i++) {
                    inverseWeights[i] = 1.0 / Math.max(vcg[i], EPSILON);
                }

            case "bids":
                inverseWeights = new double[6];
                for (int i = 0; i < 6; i++) {
                    inverseWeights[i] = 1.0 / Math.max(IR[i], EPSILON);
                }
                break;
        ...
        System.out.println(solverClient.getName());
    }
