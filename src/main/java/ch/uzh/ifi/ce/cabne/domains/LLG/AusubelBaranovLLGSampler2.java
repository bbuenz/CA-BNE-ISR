...
public class AusubelBaranovLLGSampler2 extends BidSampler<Double, Double> {
	final double alpha, gamma;
	int hack1, hack2;
	double prevV;
	double[][] cache2 = new double[500000][];
	double[][] cache1 = new double[500000][];
	
	public AusubelBaranovLLGSampler2(double alpha, double gamma, BNESolverContext<Double, Double> context) {
		super(context);
		this.alpha = alpha;
		this.gamma = gamma;
		prevV = -1.0;
		...
		for (int x=0; x<500000; x++) {
			cache2[x] = rngiter.next();
			cache1[x] = rngitercorr.next();
		}		
